...
def show_map():
    ...
    # Zobrazování smeru letadla a barvy markeru na základě nadmořské výšky
    for flight in flights:
        if all(isinstance(x, (int, float)) for x in flight[1:3]) and isinstance(flight[3], (int, float)): 
            latitude = flight[1]
            longitude = flight[2]
            heading = flight[5]
            altitude = flight[3]
            ...
            folium.RegularPolygonMarker(location=[latitude, longitude],
                                        fill_color=color, number_of_sides=3, radius=5, rotation=heading).add_to(m)
